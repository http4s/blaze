...
import org.http4s.blaze.http.http20.{TupleHeaderEncoder, TupleHeaderDecoder, BasicHttpStage, Http2ServerHubStage}
...
object ProtocolSelector {
  def apply(engine: SSLEngine, service: HttpService, maxNonbodyLength: Int): ALPNPipelineSelector = {
    
    def select(s: String): LeafBuilder[ByteBuffer] = s match {
    case HTTP2 => LeafBuilder(http2Stage(service, maxNonbodyLength))
  ...
  private def http2Stage(service: HttpService, maxHeadersLength: Int): TailStage[ByteBuffer] = {

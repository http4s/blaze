...
import org.http4s.blaze.http.http2.{FrameListener, Result, Priority}
...
private[http2] class MockFrameListener(inHeaders: Boolean) extends FrameListener {
  ...
  override def onDataFrame(streamId: Int, isLast: Boolean, data: ByteBuffer, flowSize: Int): Result = ???

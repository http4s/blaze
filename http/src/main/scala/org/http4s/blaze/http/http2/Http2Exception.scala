...
final case class Http2StreamException(stream: Int, code: Long, msg: String) extends Http2Exception(msg)
...
object Http2Exception {
  ...
  final class ErrorGenerator private[http2](val code: Long, val name: String) {
    ...
    override val toString: String = s"$name(0x${Integer.toHexString(code.toInt)})"
  }

  def errorGenerator(code: Long): ErrorGenerator = exceptionsMap.get(code) match {
    case Some(gen) => gen
    case None => new ErrorGenerator(code, s"UNKNOWN(0x${Integer.toHexString(code.toInt)})")
  }

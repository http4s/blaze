...
class BasicConnectionMonitor extends ConnectionMonitor {
  ...
  override protected def connectionAccepted(): Unit = { connections.incrementAndGet(); () }
  override protected def connectionClosed(): Unit = { connections.decrementAndGet(); () }
  override protected def bytesInbound(n: Long): Unit = { inboundBytes.addAndGet(n); () }
  override protected def bytesOutBound(n: Long): Unit = { outboundBytes.addAndGet(n); () }

  /** Get the inbound bytes, outbound bytes, and the number of live connections */
  def getStatus(): (Long, Long, Int) = (inboundBytes.get, outboundBytes.get, connections.get)
}

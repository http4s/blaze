...
class ResponseParser extends Http1ClientParser {
  ...
  def parseResponse(buffs: Seq[ByteBuffer]): (Status, Set[Raw], String) = {
    val b =
  ...
  def parseResponseBuffer(buffer: ByteBuffer): (Status, Set[Raw], String) = {
    ...
    val headers: Set[Raw] = this.headers
      .result()
      .toSet
